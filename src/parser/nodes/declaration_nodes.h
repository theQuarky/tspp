...
class VarDeclNode : public DeclarationNode {
 public:
  ...
  tokens::TokenType getStorageClass() const {
...
class ParameterNode : public DeclarationNode {
 public:
  ...
  ExpressionPtr getDefaultValue() const {
...
class ClassDeclNode : public DeclarationNode {
 public:
  ClassDeclNode(const std::string &name,
      ...
      : DeclarationNode(name, loc),
        classModifiers_(std::move(classModifiers)),
        baseClass_(std::move(baseClass)),
  ...
  const std::vector<TypePtr> &getInterfaces() const {
    return interfaces_;
  }
  const std::vector<DeclPtr> &getMembers() const {
    return members_;
  }
...
class ConstructorDeclNode : public DeclarationNode {
 public:
  ConstructorDeclNode(tokens::TokenType accessModifier,
                      ...
                      const core::SourceLocation &loc)
      : DeclarationNode("constructor", loc),
        accessModifier_(accessModifier),
        parameters_(std::move(parameters)),
        body_(std::move(body)) {}

  tokens::TokenType getAccessModifier() const {
...
class MethodDeclNode : public DeclarationNode {
 public:
  MethodDeclNode(const std::string &methodName,
                 ...
                 const core::SourceLocation &loc)
      : DeclarationNode(methodName, loc),
        accessModifier_(accessModifier),
        parameters_(std::move(parameters)),
        returnType_(std::move(returnType)),
        throwsTypes_(std::move(throwsTypes)),
...
class EnumDeclNode : public DeclarationNode {
 public:
  ...
  TypePtr getUnderlyingType() const {
    return underlyingType_;
  }
  const std::vector<EnumMemberPtr> &getMembers() const {
    return members_;
  }
 ...
 public:
  PropertySignatureNode(const std::string &name,
      ...
      : DeclarationNode(name, loc),
        accessModifier_(accessModifier),
        type_(std::move(type)),
        hasGetter_(hasGetter),
        hasSetter_(hasSetter) {}
